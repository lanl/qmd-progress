...
module gpmdcov_kernel_mod
  ...
  type, public :: kernel_type
    ...
    character(20) :: initialMixingWith
...
contains
  ...
  subroutine gpmdcov_parseKernel(kernel,filename)
    ...
    integer, parameter :: nkey_char = 2, nkey_int = 3, nkey_re = 1, nkey_log = 3
    ...
    character(len=50), parameter :: keyvector_log(nkey_log) = [character(len=100) :: &
         'KernelMixing=','BuildAlways=','UpdateAfterBuild=']
  ...
  subroutine gpmdcov_getKernel_byParts(nats,mysyprt,mysyprtk)
    use gpmdcov_vars
    use gpmdcov_writeout_mod
    use prg_response_mod
    real(dp), allocatable :: chargePertVect(:)
    ...
    write(*,*)"getKernel_byParts_cont",getKernel_byParts_cont
...
!      mysyprtk(ipt)%estr%ker = 0.0_dp
!      do m = 1,coreSize
!        mysyprtk(ipt)%estr%ker(m,m) = -0.5_dp
!      enddo
    enddo
