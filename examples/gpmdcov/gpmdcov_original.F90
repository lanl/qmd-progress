...
contains
  ...
  subroutine gpmd_DM_Min(Nr_SCF,nguess,mix)
   ...
   if(lt%verbose >= 2 .and. myRank == 1)then
      call bml_print_matrix("ham_bml",syprt(ipt)%estr%ham,0,6,0,6)
      call bml_print_matrix("ham0_bml",syprt(ipt)%estr%ham0,0,6,0,6)
      call bml_print_matrix("ham0_over",syprt(ipt)%estr%over,0,6,0,6)
    endif
      ...
      tch = sum(nguess)
      write(*,*)"Total charge after qmix",tch, tch1
  ...
  subroutine gpmd_MDloop()
    ...
    do mdstep = 1,lt%mdsteps
      ...
      !> SCF loop
      write(*,*)"sy%net_charge bef d min",n
      if(Nr_SCF_It.ne.0)call gpmd_DM_Min(Nr_SCF_It,n,.true.)
      write(*,*)"sy%net_charge aft d min",n
  ...
  subroutine gpmd_RhoSolver(orthoh_bml,orthop_bml)
    ...
    elseif(lt%method.EQ."Diag")then
      ...
      call prg_build_density_T_Fermi(orthoh_bml,orthop_bml,lt%threshold, lt%kbt, Ef)
