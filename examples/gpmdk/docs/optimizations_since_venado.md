...
> Caveat: most HPC-optimization commits recorded *what* changed but not
...
> qualitative and should be backed by fresh benchmarks.
...
Design pattern worth highlighting: a persistent-GPU-data strategy
...
## 7. Adaptive time step + XLBO (the feature this work centers on)
...
### 7a. LATE-BREAKING (Aug 2026, uncommitted analysis — verify before use)
...
2. **Reversible-backtracking scheme is worse, not better.** A symmetric-split (both-endpoint,
   ...
   largely gone. **Do not feature the reversible scheme as a win** unless the
...
Caveats: (a) these are NVTX wall-time sums, not isolated CUDA-kernel device
...
- **Benchmarks to (re)generate:** the SEDACS NVTX profiles already give per-region
  ...
  DM_min 1.77×, plus the Ewald scale crossover). Still needed: strong/weak
