...
> Caveat: many HPC-optimization commits recorded *what* changed but not
...
> should be backed by fresh benchmarks.
...
Caveats: (a) these are **NVTX-tag wall times per MD step** (mean over the
...
- **Benchmarks to (re)generate:** `profile_log.csv` already gives per-routine speedups (see
  ...
  Still needed: strong/weak scaling, ns/day vs system size, memory vs
