module gpmdcov_neighbor_mod
  ...
  !> Maximum atomic number density (atoms/Ang^3) used to size neighbor lists.
  !! Set once from the GPMD{} MaxDensity= input keyword (gpmdt%maxdensity) after
  !! parsing; defaults to 0.25 so behavior is unchanged if never assigned.
  real(dp), public :: maxdensity = 0.25_dp
  ...
  public ::  gpmdcov_get_nlist_box_indices, gpmdcov_build_nlist_sedacs
...
!      write(*,*)ix,iy,iz,boxOfI(i),coords(:,i)
  ...
  subroutine gpmdcov_build_nlist_sedacs(coords,lattice_vectors,rcut,nll,verbose,rank,numranks)
    ...
    real(dp)                             ::  maxx, maxy, maxz
    real(dp)                             ::  boxSize,boxSizeX,boxSizeY,boxSizeZ
    real(dp), allocatable, intent(in)    ::  coords(:,:), lattice_vectors(:,:)
    real(dp), intent(in)                 ::  rcut
    logical, allocatable :: inSurf(:)
    type(neighlist_type), intent(inout)  ::  nll
