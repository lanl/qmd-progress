...
!    write(*,*)"evalsInEachCHAtRank",norbsInEachCHAtRank
    ...
    if(err .eqv. .true.)then
      call gpmdcov_musearch_bisec(evalsAll,dvalsAll,beta,nocc,10d-10,Ef,myRank,egap_glob,lt%verbose)
    endif
  ...
  !!
  subroutine gpmdcov_musearch_bisec(evals,dvals,beta,noc,tol,mu,rank,egap,verbose)
    use gpmdcov_writeout_mod
    integer                  ::  i, m
    integer                  ::  norb
