module gpmdcov_EnergAndForces_mod
  ...
  !!
  subroutine get_nonortho_coul_forces_local(nats, norb, dSx_bml,dSy_bml,dSz_bml,&
  ...
  end subroutine get_nonortho_coul_forces_local
...
!             dH0x(hindex(1,i):hindex(2,i),hindex(1,j):hindex(2,j)) = (dH0x(hindex(1,i):hindex(2,i),hindex(1,j):hindex(2,j)) - H0xm(hindex(1,i):hindex(2,i),hindex(1,j):hindex(2,j)))/(2.0_dp*dx)
!             dH0y(hindex(1,i):hindex(2,i),hindex(1,j):hindex(2,j)) = (dH0y(hindex(1,i):hindex(2,i),hindex(1,j):hindex(2,j)) - H0ym(hindex(1,i):hindex(2,i),hindex(1,j):hindex(2,j)))/(2.0_dp*dx)
!             dH0z(hindex(1,i):hindex(2,i),hindex(1,j):hindex(2,j)) = (dH0z(hindex(1,i):hindex(2,i),hindex(1,j):hindex(2,j)) - H0zm(hindex(1,i):hindex(2,i),hindex(1,j):hindex(2,j)))/(2.0_dp*dx)
...
!                  blk(:,:) = 0.0_dp
   ...
   !            stop
 end subroutine get_dSKBlock_inplace
  ...
  subroutine gpmdcov_EnergAndForces(charges)
    ...
    real(dp) :: deltas(3), EDisp, EBand, ECoulRecip
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    do i = 1,sy%nats
      ...
      ECoulRecip = ECoulRecip + charges(i)*coul_pot_k(i);
    enddo
    ...
    if((myRank == 1) .and. (lt%verbose >= 2))then
      ...
      write(*,*)"Energy Coulomb (Per electron) = ", ECoul/sy%estr%nel
      write(*,*)"Energy Band =", EBand
      write(*,*)"Energy Coulomb Local (Per electron) = ", (ECoul - ECoulRecip)/sy%estr%nel
      write(*,*)"Energy Band (Per electron) =", EBand/sy%estr%nel
