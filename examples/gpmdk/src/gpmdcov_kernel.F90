...
!!!   System -> Cores+halos(0)
  ...
  subroutine gpmdcov_rankN_update_byParts(myqn,myn,mysyprt,mysyprtk,maxRanks,KK0Res,newnl_in)
    ...
    logical                       :: newnl
#ifdef USE_OFFLOAD
    type(c_ptr) :: ptaux_bml_c_ptr, zq_bml_c_ptr
    integer :: ld
    real(c_double), pointer :: ptaux_bml_ptr(:,:), zq_bml_ptr(:,:)
#endif
...
!stop
    ...
    do irank = 1, mRanks
      ...
      do iptt = 1,gpat%TotalParts
        ...
        enddo
#ifdef USE_NVTX
        call gpmdStartRange("BML_math",2)
#endif
...
!        call gpmdcov_reallocate_denseBmlRealMat(ptham_bml,norbs)
        ...
        ptaux_bml_c_ptr = bml_get_data_ptr_dense(ptaux_bml)
        zq_bml_c_ptr = bml_get_data_ptr_dense(zq_bml)
        ld = bml_get_ld_dense(dPdMu_bml)

        call c_f_pointer(ptaux_bml_c_ptr,ptaux_bml_ptr,shape=[ld,norbs])
        call c_f_pointer(zq_bml_c_ptr,zq_bml_ptr,shape=[ld,norbs])

        !$acc enter data copyin(dPdMu(:))
        !$acc parallel loop gang vector collapse(2) deviceptr(ptaux_bml_ptr,zq_bml_ptr)
        do ii = 1,norbs
           do jj = 1,norbs
              ptaux_bml_ptr(ii,jj) = zq_bml_ptr(ii,jj)*dPdMu(ii)
           enddo
        enddo
...
#endif
#ifdef USE_NVTX
        call gpmdStartRange("Calculate V",7)
#endif
...
#endif
#ifdef USE_NVTX
        call gpmdStartRange("Deallocations",5)
#endif
  ...
  !Deallocations 
    ...
    call gpmdEndRange
#endif
   call gpmdcov_msI("gpmdcov_get_kernel_byParts","Time for rankN_update_byParts &
             &"//to_string(mls() - mls_v)//" ms",lt%verbose,myRank)
