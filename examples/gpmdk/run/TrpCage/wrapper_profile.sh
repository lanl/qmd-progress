...
#	nsys profile -e NSYS_MPI_STORE_TEAMS_PER_RANK=1 -t mpi,openmp,cuda,nvtx --mpi-impl=mpich --delay=90 $RUN input.in >& out_0
        nsys profile -e NSYS_MPI_STORE_TEAMS_PER_RANK=1 -c cudaProfilerApi  -t mpi,openmp,cuda,nvtx --mpi-impl=mpich -o report_%q{SLURM_PROCID} $RUN input.in >& out_$SLURM_PROCID
