...
program gpsolve
  ...
  real(dp) :: ef,sparsity,dec,mlsi, smooth_maxCH, sumCubes
...
!    endif
  ...
  do i = gpat%localPartMin(myRank), gpat%localPartMax(myRank)
    ...
    !   write(*,*)"CH index from part",gpat%sgraph(i)%core_halo_index
    call bml_zero_matrix("dense",bml_element_real,dp,inorbs,inorbs,syprt(i)%estr%ham)
    ! call bml_print_matrix("syprt(i)%estr%ham",syprt(i)%estr%ham,0,10,0,10)
    ...
    call bml_matrix2submatrix(ham_bml, syprt(i)%estr%ham, vector, &
         & inorbs)
    !call bml_print_matrix("syprt(i)%estr%ham",syprt(i)%estr%ham,0,5,0,5)
