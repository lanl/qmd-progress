...
program mdresponse
  ...
  call write_system(sy,adjustl(trim(lt%jobname))//"_SCFcharges","pdb")
...
contains
  ...
  subroutine gpmd_FirstCharges()
    ...
    call homolumogap(vv, icount, pp, gbnd(1), gbnd(2), ehomo, elumo, egap,lt%verbose)
    if (printRank() .eq. 1) then
  ...
  subroutine gpmd_SCFLoop(Nr_SCF,nguess)
    ...
    do i=1,Nr_SCF
      ...
      !    if (printRank() .eq. 1) then
      write(*,*)""; write(*,*)"In orthogonalize H ..."
      !    endif
      call timer_start(ortho_timer)
      call orthogonalize(ham_bml,zmat_bml,orthoh_bml,&
        lt%threshold,lt%bml_type,lt%verbose)
  ...
  subroutine gpmd_EnergAndForces(charges)
    ...
    call timer_start(dyn_timer,"Get pair pot")
    ...
    call timer_start(dyn_timer,"get_dH and get_dS")
 ...
 ! stop
    call timer_start(dyn_timer,"SK forces")
    ...
    call get_pulayforce(sy%nats,zmat_bml,ham_bml,rho_bml,&
  ...
  subroutine gpmd_MDloop()
    ...
    do MD_step = 1,lt%mdsteps
      ...
      !> Update positions
      call timer_start(dyn_timer,"Update positions")
      ...
      !> Get new H0 and S
      call timer_start(dyn_timer,"Build H0 and S")
      call get_hsmat(ham_bml,over_bml,sy%coordinate,sy%lattice_vector,sy%spindex,&
        tb%norbi,hindex,onsitesH,onsitesS,intPairsH,intPairsS,lt%threshold)
      call timer_stop(dyn_timer,1)
      ...
      call xlbo_nint(sy%net_charge,n,n_0,n_1,n_2,n_3,n_4,n_5,MD_step,xl)
     ...
     if(lt%verbose.GE.1)then
      ...
      call timer_start(dyn_timer,"Get Hscf")
      call get_hscf(ham0_bml,over_bml,ham_bml,sy%spindex,hindex,tb%hubbardu,n,&
        coul_pot_r,coul_pot_k,lt%mdim,lt%threshold)
      call timer_stop(dyn_timer,1)
...
!       endif
      ...
      call deorthogonalize(orthop_bml,zmat_bml,rho_bml,&
        lt%threshold,lt%bml_type,lt%verbose)
      call timer_stop(deortho_timer,1)
#ifdef DO_MPI_BLOCK
      call allGatherParallel(rho_bml)
#endif
