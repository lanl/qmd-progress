...
contains
  ...
  !! \param Inv_bml Inverses generated by algorithm.  
  ...
  !! \param SCF_IT The current SCF iteration. 
  !! \param occiter Counts the total nr of DM calculations during MD. 
  !! See \cite{niklasson2003}
  subroutine prg_implicit_fermi_save_inverse(Inv_bml, h_bml, p_bml, nsteps, nocc, &
       mu, beta, occErrLimit, threshold, tol,SCF_IT, occiter)
    ...
    do while (occErr .gt. occErrLimit .or. muadj .eq. 1) 
      iter = iter + 1
      muadj = 0
      write(*,*) 'mu =', mu
      ! Normalization
  ...
  subroutine prg_implicit_fermi(h_bml, p_bml, nsteps, k, nocc, &
    ...
    call bml_print_matrix("h_bml",h_bml,1,10,1,10)
    call bml_print_matrix("p_bml",p_bml,1,10,1,10)
    write(*,*) nsteps, k, nocc, &
       mu, beta, method, osteps, occErrLimit, threshold, tol
    !stop
    allocate(trace(2))
  ...
  !! \param H1_bml Input First order perturbation of H0.
  !! \param P0_bml Output density matrix.
  !! \param P1_bml Output First order density matrix response.
  !! \param nsteps Number of recursion steps.
  ...
  subroutine prg_newtonschulz(a_bml, ai_bml, r_bml, tmp_bml, d_bml, I_bml, tol, threshold)
