...
!! More information about the teory can be found at \cite Niklasson2005 and Niklasson2015
module response_mod
  ...
  !Final variables passed to the response module
  ...
  public :: compute_response_FD, compute_polarizability
...
contains
  ...
  subroutine parse_response(RespData, filename)
    ...
    call parsing_kernel(keyvector_char,valvector_char&
      ...
      keyvector_log,valvector_log,trim(filename))
  ...
  !! \param factor Unit conversion factor (use 1.0 is no conversion is requiered). 
  ...
  !! The units of the directional polarizability are determined by the unis of 
  !! the perturbation and Hamiltonian. This equation can be found in \cite Weber2005 equation 4a.
  !! Note that in equation 4a of the reference there is a 2 that acount for the double occupancy
  ...
  !! If coordinates are in \f$ \AA \f$ and charges are in fractions of electron, then 
  !! transformation ea2debye form LATTE lib can be used to change units to Debye.
  subroutine compute_polarizability(resp_bml,pert_bml,polarizability,factor,verbose)
    ...
    type(bml_matrix_t), intent(in)     ::  pert_bml, resp_bml
    type(bml_matrix_t) :: aux_bml
    
    if(verbose.gt.1)write(*,*)"Computing polarizability ..."

    call bml_copy_new(pert_bml,aux_bml)
    call bml_multiply(resp_bml,pert_bml,aux_bml,1.0_dp,0.0_dp,0.0_dp)
  ...
  subroutine pert_from_file(pert_bml,norb)
    implicit none 
    integer :: Norb, verbose
    type(bml_matrix_t), intent(inout) :: pert_bml
    if(verbose.GT.1) write(*,*)'Reading perturbation V from file ...'

  end subroutine pert_from_file


  ! Matlab code for RS PT   
  !   function [D1] = RS_PRT(C,E,H1,mu)   
  !    N = max(size(H1));
  !    C1 = 0*C;
  !    for i = 1:N
  !      if E(i,i) < mu
  !        for j = 1:N
  !           if j ~= i
  !             H1Tmp = (C(:,j)'*H1*C(:,i)/(E(i,i)-E(j,j)));
  !             C1(:,i) = C1(:,i) + H1Tmp*C(:,j);
  !          end
  !       end
  !     end
  !   end
  !   D1 = C1*C' + C*C1';  % Calculated using Rayleigh Schrodinger Pertrubations theory

  !> Computes the first order response density matrix using Rayleigh Schrodinger Pertrubations 
  !! theory
  ...
  !! \param pert_bml Perturbation in bml format (\f$ H^{(1)} \f$).
  !! \param resp_bml First order response to the perturbation (\f$ \rho^{(1)} \f$).
  ...
  subroutine compute_response_RS(ham_bml,pert_bml,resp_bml,lambda&
    ...
    type(bml_matrix_t), intent(in)     ::  ham_bml, pert_bml
    type(bml_matrix_t), intent(inout)  ::  resp_bml
    ...
    !Eigenvectors and eigenalues of H    
    ...
    call bml_multiply(aux_bml,pert_bml,aux1_bml,1.0_dp,0.0_dp,threshold)
    ...
    call bml_multiply(aux_bml,aux2_bml,resp_bml,1.0_dp,0.0_dp,threshold)    
    ...
    call bml_multiply(aux1_bml,aux_bml,resp_bml,1.0_dp,1.0_dp,threshold)
    call bml_scale(1.0_dp,resp_bml)

    write(*,*)"Trace",bml_trace(resp_bml)
  ...
  !! \warning This works only for the orthogonalized form of ham_bml.
  !! \warning The response must be in the orthogonalized form. 
  subroutine compute_response_FD(ham_bml,pert_bml,resp_bml,delta&
      ,bndfil,threshold,verbose)
    implicit none 
    character(20)                      ::  bml_type
    integer                            ::  i, j, l, norb
  ...
  !! field to a region of the system. In this implemetation \f$ e= 1 \f$ 
  ...
  subroutine pert_constant_field(field,intensity,coordinate,lambda&
      ,pert_bml,threshold,spindex,norbi,verbose,over_bml)
    implicit none    
    ...
    if(bml_get_N(pert_bml) < 0) stop "bml_pert not allocated"

    call bml_set_diagonal(pert_bml,diag,threshold)

    deallocate(diag)

    if(present(over_bml))then  !(S*V + V*S)/2
      call bml_copy_new(pert_bml,aux_bml)
      call bml_multiply(over_bml,aux_bml,pert_bml,0.5_dp, 0.0_dp,threshold)
      call bml_multiply(aux_bml,over_bml,pert_bml,0.5_dp,1.0_dp,threshold) 
      call bml_deallocate(aux_bml)
    endif
  ...
  !! \warning This works only for the orthogonalized form of ham_bml.
  !! \warning The response must be in the orthogonalized form. 
  subroutine compute_response_SP2(ham_bml,pert_bml,resp_bml,rho_bml,lambda&
    ...
    type(bml_matrix_t), intent(in)     ::  ham_bml, pert_bml
    type(bml_matrix_t), intent(inout)  ::  resp_bml, rho_bml
    ...
    if(bml_get_N(resp_bml).LE.0)then 
      call bml_zero_matrix(bml_matrix_dense,bml_element_real,dp,norb,norb,resp_bml)       
    endif

    occ = bndfil*float(norb) 

    call bml_copy(ham_bml, rho_bml)
    call bml_copy(pert_bml,resp_bml)    
    ...
    call bml_scale(alpha, resp_bml) !Same as SP2 but with the perturbation.
    
    allocate(trace(2))
    trx = bml_trace(rho_bml)
    trd = bml_trace(resp_bml)
    ...
    call bml_copy_new(resp_bml, aux_bml)

    do while (breakloop .eq. 0 .and. iter .lt. maxsp2iter)
      iter = iter + 1
          
    call bml_print_matrix("resp_bml",resp_bml,0,1,0,1)
      ...
      call bml_multiply(rho_bml, resp_bml, aux_bml, 1.0d0, 0.0d0)
      call bml_multiply(resp_bml,rho_bml, aux_bml, 1.0d0, 1.0d0)
      ...
      elseif(limdiff .lt. -idemtol) then
        ...
        call bml_copy(aux_bml, resp_bml)
  ...
  call bml_scale(2.0_dp, resp_bml)    
