...
contains
  ...
  subroutine compute_response_FD(ham_bml,prt_bml,rsp_bml,delta&
    ...
    !! - \f$ H^+ = H^{(1)} + \delta I \f$
    call bml_add(1.0_dp, aux_bml, delta, prt_bml, threshold)
    ...
    !! - \f$ H^- = H^{(1)} - \delta I \f$
    call bml_add(1.0_dp, aux_bml, -2.0_dp*delta, prt_bml, threshold)
  ...
  subroutine compute_response_SP2(ham_bml,prt_bml,rsp_bml,rho_bml,lambda&
    ...
    do while (breakloop .eq. 0 .and. iter .lt. maxsp2iter)
       ...
       if (limdiff .ge. idemtol) then
          ...
          call bml_add(2.0_dp,rsp_bml,-1.0_dp, aux_bml, threshold)
