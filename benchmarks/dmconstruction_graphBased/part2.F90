...
program gpsolve
  ...
  !call bml_copy(ham_bml, g_bml) !Alternative direct use of ham
  ...
  ! Call API
  mlsi = mls()
  call prg_build_densityGP_T0(ham_bml, g_bml, rho_bml, threshold_g, bndfil, Ef, nparts)
  ...
  if(printRank() == 1)write(*,*)"|rhoGP-rho|/((1-sparsity)*N2)",bml_fnorm(aux_bml)/((1.0_dp - sparsityRho)*norbs*norbs)
